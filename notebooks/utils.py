...
def get_data(path, target_size=(224,224)):
    ...
    batches = get_batches(path, gen=img_gen, shuffle=False, batch_size=1, class_mode=None, target_size=target_size)
    return np.concatenate([batches.next() for i in range(batches.nb_sample * 2)])
...
def get_classes(path):
    ...
    val_batches = get_batches(path+'valid', gen=img_gen, shuffle=False, batch_size=1)
